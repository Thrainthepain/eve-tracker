...
# Last Updated: 2025-05-04 04:14:48

# Reset and clear terminal
clear

# Color codes for better readability - Using tput for broader compatibility
...
print_message() {
  # Skip color codes processing entirely
  echo "$2"
}
...
start_containers() {
  ...
  if [ $? -ne 0 ]; then
    echo "Failed to start containers. Please check docker logs for details."
  ...
  while [ $retry_count -lt $max_retries ]; do
    ...
    echo "Not all containers are running yet. Waiting (attempt $((retry_count+1))/$max_retries)..."
    retry_count=$((retry_count+1))
  done
  
  if [ $running_containers -lt $expected_containers ]; then
    echo "Not all containers are running. Expected $expected_containers but found $running_containers."
    echo "Running containers:"
    $sudo_cmd docker ps
...
verify_application() {
  ...
  if curl -s --connect-timeout 5 "$backend_url" | grep -q "status\|healthy\|UP"; then
    ...
    echo "Backend API is not responding. Checking container logs:"
    $sudo_cmd docker logs eve-tracker-backend
    
    echo "This might be normal if the application is still initializing."
    echo "You can try again manually: curl -v $backend_url"
  fi
  
  # Check if frontend is accessible
  frontend_url="${SERVER_PROTOCOL}://${FULL_DOMAIN}:${FRONTEND_PORT}"
  echo "You can access the frontend at: $frontend_url"
  echo "Note: Frontend might take a few more moments to fully initialize."
}
...
display_welcome_banner() {
  echo "╔═══════════════════════════════════════════════════════════════╗"
  echo "║                                                               ║"
  echo "║           EVE Online Character Tracker - Docker Setup          ║"
  echo "║                                                               ║"
  echo "║  Created by: ThrainthepainUsing                               ║"
  echo "║  Last Updated: 2025-05-04 04:14:48                          ║"
  echo "║                                                               ║"
  echo "╚═══════════════════════════════════════════════════════════════╝"
}
...
main() {
  ...
  if [ -f .env ]; then
    ...
    echo "Frontend: ${SERVER_PROTOCOL}://${FULL_DOMAIN}:${FRONTEND_PORT}"
    echo "Backend API: ${SERVER_PROTOCOL}://${FULL_DOMAIN}:${BACKEND_PORT}/api"
    echo "Admin login is available once you sign in with an EVE character"
    echo "and manually set the first user as admin in the database."
  else
    echo "EVE Online Character Tracker is now running!"
  ...
  echo "To stop the application, run: $sudo_cmd $compose_cmd down"
  echo "To view logs, run: $sudo_cmd $compose_cmd logs -f"
  echo "To restart after system reboot, run: $sudo_cmd $compose_cmd up -d"
}
